  ...
  if ( items.length == 0 ) {
    ...
    if ( summonerResponse ) {
      if ( summonerResponse.status == 200 ) {
        console.log(`요청 정상 : http://127.0.0.1:8080/api/summoner/${encodeURI(summonerName)}`);
        ...
        if ( gameResponse ) {
          console.log(`요청 정상 게임수집 : http://127.0.0.1:8080/api/game/data/summoner/${id}/${paramsStartDate}/${paramsEndDate}`);          
