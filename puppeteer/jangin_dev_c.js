...
async function getSummoner() {
  if ( items.length == 0 ) {
    if ( complteItems.length != 0 ) {
      fs.writeFileSync(`opgg_ranking_complete_dev/page.${page}.json`,JSON.stringify(complteItems));
      complteItems = [];
    }
    ...
    const summonerResponse = await apiGetCall(`http://118.67.128.59:8080/api/summoner/${encodeURI(summonerName)}`);
    let id = 0;
    if ( summonerResponse ) {
      if ( summonerResponse.status == 200 ) {
        summonerResponseStatus = summonerResponse.status;
        id = summonerResponse.data.id;
        const gameResponse = await apiPutCall(`http://118.67.128.59:8080/api/game/summoner/${id}`);
        if ( gameResponse ) {
